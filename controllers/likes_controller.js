...
likes.post('/like', isAuthenticated, async(req, res) => {
    try {
        const likePost = await pool.query("UPDATE posts SET likes = likes + 1 WHERE post_id = $1;",[req.body.postID])

        const createLike = await pool.query("INSERT INTO likes (username, post_id) VALUES($1,$2) returning *", [req.session.currentUser[0], req.body.postID])
        ...
        const newNotification = await pool.query("INSERT INTO notifications (current_session_user, action, username, post_id, notification_date) VALUES($1, $2, $3, $4, $5)", [current_session_user, action, req.body.postUsername, req.body.postID, notification_date])
...
likes.post('/unlike', isAuthenticated, async(req, res) => {
    try {
        const unlikePost = await pool.query("UPDATE posts SET likes = likes - 1 WHERE post_id = $1", [req.body.postID])

        // console.log({unlikePost});

        const createUnlike = await pool.query("DELETE FROM likes WHERE username = $1 AND post_id = $2", [req.session.currentUser[0], req.body.postID])
        // console.log({createUnlike});

        //delete like notification
        const deleteNotification = await pool.query("DELETE FROM notifications WHERE current_session_user = $1 AND action = $2 AND username = $3 AND post_id = $4", [req.session.currentUser[0], 'liked', req.body.postUsername, req.body.postID])
